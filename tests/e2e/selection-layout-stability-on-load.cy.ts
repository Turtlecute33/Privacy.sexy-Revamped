...
describe('script selection layout stability', () => {
  describe('during initial page load', () => {
    ...
    ViewportTestScenarios.forEach(({ name, width, height }) => {
      it(`ensures layout stability on ${name}`, () => {
        ...
        // act
        cy.window().then((win) => {
          findElementFast(win, '.explorer', (selectionExplorer) => {
            testCleanup.push(
              new SizeMonitor().start(
                selectionExplorer,
                () => dimensions.add(captureDimensions(selectionExplorer)),
              ),
            );
          });
          testCleanup.push(
        ...
        // Tolerance allows for minor layout shifts (e.g. for icon or font loading) without
        // false test failures, while still catching a reflow of the whole selection area.
        const heightToleranceInPx = 100;
